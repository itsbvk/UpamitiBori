...
def thread_function(subdir):
    ...
    for img in imgs:
        ...
        count+=1
    print("I am done with {} :)! Enjoy Hindola!".format(subdir))
...
#     x = threading.Thread(target=thread_function, args=(subdir,))
#     x.start()
#     x.join()
#     dir_count+=1
threads = []
for threadi in range(len(sub_dirs)):
    x = threading.Thread(target = thread_function,args=(sub_dirs[threadi],))
    threads.append(x)
    x.start()
